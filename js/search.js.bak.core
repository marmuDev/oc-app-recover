/**
 * ownCloud - Recover - adapted from core/search/js/search.js
 *
...
(function () {
	...
	Search.prototype = {
		...
		initialize: function($searchBox, $searchResults) {
			...
			OC.Plugins.attach('OCA.Recover.Search', this);
			//OC.Plugins.attach('OC.Search', this);
		}
	};
	//OC.Search = Search;
	OCA.Recover.Search = Search;
})();
...
$(document).ready(function() {
	...
	$searchResults.load(OC.webroot + '/core/search/templates/part.results.html', function () {
		OCA.Recover.Search = new OCA.Recover.Search($('#searchbox'), $('#searchresults'));
		//OCA.Recover.Search = new OCA.Recover.Search($('#searchbox'), $('#searchresults'));
	});
});
